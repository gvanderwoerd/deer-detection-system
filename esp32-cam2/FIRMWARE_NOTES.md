# ESP32-CAM Firmware Notes - CAMERA 2 (Back Yard)

**Last Updated:** 2026-05-21
**Status:** ✅ Working and tested
**Camera:** Camera 2 (Back Yard)
**Hostname:** esp32cam-back.local
...
```cpp
MDNS.begin("esp32cam");
// Accessible via: http://esp32cam-back.local:81/
```
...
**Primary (Definitive):**
```
/mnt/linux-data/deer-detection-system/esp32-cam2/src/main.cpp
```
...
### Using PlatformIO:
```bash
cd /mnt/linux-data/deer-detection-system/esp32-cam2
...
4. Test stream: `http://192.168.1.101:81/stream` or `http://esp32cam-back.local:81/stream`
...
2. Try mDNS: `http://esp32cam-back.local:81/stream`
3. Check router DHCP/static IP settings
4. Ping test: `ping 192.168.1.101`
...
Edit in `main.cpp`:
```cpp
const char* MDNS_HOSTNAME = "esp32cam-back";  // Change for multiple devices
```
